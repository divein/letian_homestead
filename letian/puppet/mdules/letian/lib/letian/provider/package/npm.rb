require 'letian/provider/package'


Letian::Type.type(:package).provide :npm, :parent => Puppet::Provider::Package do
  desc "npm is a package management for node.js. This provider only handles global packages."

  has_feature :versionable

  if Letian::Util::Package.versioncmp(Puppet.version, '3.0') >= 0
  ...
  def self.npmlist
    begin
      ...
      Letian.debug("Error: npm list --json command error #{e.message}")
      @npmlist = {}
    end
  end
